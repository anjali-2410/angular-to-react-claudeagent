...
describe('Checkbox', () => {
  describe('basic rendering', () => {
    ...
    it('auto-generates a unique id when checkboxId is omitted', () => {
      ...
      expect(input.id).toMatch(/^sol-checkbox-\d+$/);
    });
  ...
  describe('checked state', () => {
    ...
    it('calls onClicked on every click including disabled prevention', async () => {
      const user = userEvent.setup();
      const clicked = vi.fn();
  ...
  describe('indeterminate state', () => {
    ...
    it('clears indeterminate on click and calls onIndeterminateChange', async () => {
      const user = userEvent.setup();
      const indeterminateHandler = vi.fn();
      render(<Checkbox indeterminate onIndeterminateChange={indeterminateHandler} />);
      const input = screen.getByRole('checkbox') as HTMLInputElement;
      expect(input.indeterminate).toBe(true);
      await user.click(input);
      expect(input.indeterminate).toBe(false);
      expect(indeterminateHandler).toHaveBeenCalled();
    });
  });
  ...
  describe('disabled state', () => {
    ...
    it('does not call onCheckedChange when disabled', async () => {
      ...
      // Clicks on disabled inputs don't fire in testing-library by default,
      // but we verify the handler is not called.
      await user.click(screen.getByRole('checkbox'));
...
describe('CheckboxGroup', () => {
  ...
  it('does not add duplicate values', async () => {
    ...
    await user.click(screen.getByRole('checkbox'));
    // Clicking a checked checkbox should uncheck it, not duplicate
  ...
  it('does not include error message inside a child checkbox', () => {
    render(
      <CheckboxGroup errorMessage="Group error">
        <Checkbox value="a" errorMessage="Checkbox error">A</Checkbox>
      </CheckboxGroup>
    );
    // The checkbox errorMessage should NOT render because it's inside a group
    // (group suppresses individual error message rendering)
