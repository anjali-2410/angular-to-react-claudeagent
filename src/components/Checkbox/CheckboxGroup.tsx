...
export interface CheckboxGroupProps {
  /** name for the group (passed to context, not rendered directly) */
  name?: string;
  ...
  required?: boolean;
  /** Array of selected values (controlled); omit to use uncontrolled mode */
  value?: string[];
  /** Text label rendered above the group */
  label?: string;
  /** Shows error border on the group label area */
  errorState?: boolean;
...
export const CheckboxGroup: React.FC<CheckboxGroupProps> = ({
  ...
  const handleCheckboxChange = (value: string, checked: boolean) => {
    ...
    // Clear required error when user selects something
